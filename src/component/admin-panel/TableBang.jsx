...
export default function TableBang({
    loading,
    activeTab,
    currentContacts,
    currentDemos,
            ...
            <div className="overflow-x-auto">
                {activeTab === 'contacts' ? (
                    <table className="w-full border-collapse min-w-[1000px]">
                        <thead>
                            <tr className="bg-[var(--bg-light)]">
                                {['#', 'নাম', 'ইমেইল', 'ফোন', 'দেশ', 'তারিখ', 'অ্যাকশন'].map(h => (
                                    <th key={h} className={`py-3.5 px-5 text-left text-[11px] font-bold text-[var(--primary-color)] uppercase tracking-wider border-b border-[var(--border-color)] ${h === 'অ্যাকশন' ? 'text-right' : ''}`}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {currentContacts.length === 0 ? (
                                <tr><td colSpan={7} className="text-center py-12 text-[var(--text-secondary)] text-sm">কোন যোগাযোগ বার্তা পাওয়া যায়নি।</td></tr>
                                ...
                                <tr key={c._id} className="admin-table-row cursor-default">
                                    ...
                                    <td className="py-3.5 px-5">
                                        <span className="inline-block py-1 px-2.5 rounded-md bg-[var(--secondary-color)]/10 text-[var(--secondary-color)] text-[11px] font-bold uppercase">{c.country}</span>
                                    </td>
                    ...
                    <table className="w-full border-collapse min-w-[1000px]">
                        <thead>
                            <tr className="bg-[var(--bg-light)]">
                                {['#', 'কোম্পানি', 'নাম', 'ইমেইল', 'ফোন', 'দেশ', 'তারিখ', 'অ্যাকশন'].map(h => (
                                    <th key={h} className={`py-3.5 px-5 text-left text-[11px] font-bold text-[var(--primary-color)] uppercase tracking-wider border-b border-[var(--border-color)] ${h === 'অ্যাকশন' ? 'text-right' : ''}`}>{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {currentDemos.length === 0 ? (
                                <tr><td colSpan={8} className="text-center py-12 text-[var(--text-secondary)] text-sm">কোন ডেমো অনুরোধ পাওয়া যায়নি।</td></tr>
                                ...
                                <tr key={d._id} className="admin-table-row cursor-default">
                                    ...
                                    <td className="py-3.5 px-5">
                                        <span className="inline-block py-1 px-2.5 rounded-md bg-[var(--secondary-color)]/10 text-[var(--secondary-color)] text-[11px] font-bold uppercase">{d.country}</span>
                                    </td>
            ...
            {activeData.length > 0 && (
                <div className="flex justify-between flex-wrap gap-[20px]  items-center px-6 py-4 border-t border-[var(--border-color)] bg-white mt-auto">
                    <p className="text-[13px] text-[var(--text-secondary)] font-medium m-0">
                        প্রদর্শন করা হচ্ছে <span className="font-bold text-[var(--primary-color)]">{indexOfFirstItem + 1}</span> থেকে <span className="font-bold text-[var(--primary-color)]">{Math.min(indexOfLastItem, activeData.length)}</span>, সর্বমোট <span className="font-bold text-[var(--primary-color)]">{activeData.length}</span> এর মধ্যে
                    </p>
